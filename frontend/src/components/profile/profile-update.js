...
export default class UpdateProfile extends Component {
    ...
    UserService.getUserBoard().then(
      (response) => {
        ...
        if (response.data.role.name === 'customer') {
          axios
            .get(
              `http://localhost:4444/customer/get-customer/${this.state.currentUser.id}`
            )
            .then((response) => {
          ...
          axios
            .get(
              `http://localhost:4444/employee/get-employee/${this.state.currentUser.id}`
            )
            .then((response) => {
  ...
  UpdateProfile(e) {
    ...
    if (this.state.customer) {
      if (
        this.state.password.localeCompare(this.state.customer.password) &&
        this.state.cpassword.localeCompare(this.state.password)
      ) {
      ...
      if (
        this.state.password.localeCompare(this.state.employee.password) &&
        this.state.cpassword.localeCompare(this.state.password)
      ) {
        console.log('Mismatch')
        alert("Passwords don't match.")
    ...
    axios.patch(`http://localhost:4444/auth/${this.state.currentUser.id}`, user)
    ...
    if (this.state.role === 'customer') {
      ...
      axios
        .patch(`http://localhost:4444/customer/${this.state.id}`, customer)
        .then((response) => {
          Swal.fire({
            position: 'center',
            icon: 'success',
      ...
      axios
        .patch(
          `http://localhost:4444/employee/update-profile/${this.state.id}`,
          employee
        )
        .then((response) => {
          Swal.fire({
                ...
                <div className="col-3" id="profile-box">
                  ...
                  <div className="text-center">
                    <button
                      ...
                      onClick={(e) => this.navigateUpdateImage(e)}
                    >
                      Update Image
                    </button>
                  </div>
